...
NdefRecord_t DetectNdefRecordType(unsigned char *pNdefRecord) {
  ...
  if (pNdefRecord[0] & NDEF_RECORD_SR_MASK) {
    ...
    record.recordPayloadLength = (pNdefRecord[2] << 24) +
                                 (pNdefRecord[3] << 16) +
                                 (pNdefRecord[4] << 8) + pNdefRecord[5];
    typeField = 6;
  }
  ...
  if (pNdefRecord[0] & NDEF_RECORD_IL_MASK) {
    record.recordPayload =
        pNdefRecord + typeField + pNdefRecord[1] + 1 + pNdefRecord[typeField];
    typeField++;
  } else {
  ...
  /* Media Record Type ?*/
    ...
    if ((memcmp(&pNdefRecord[typeField], "text/x-vCard", pNdefRecord[1]) ==
         0x0) ||
        (memcmp(&pNdefRecord[typeField], "text/vcard", pNdefRecord[1]) ==
         0x0)) {
...
unsigned char *GetNextRecord(unsigned char *pNdefRecord) {
  ...
  if (!(pNdefRecord[0] & NDEF_RECORD_ME_MASK)) {
    ...
    if (pNdefRecord[0] & NDEF_RECORD_SR_MASK) {
      /* ID present ?*/
      if (pNdefRecord[0] & NDEF_RECORD_IL_MASK)
        temp = (pNdefRecord + 4 + pNdefRecord[1] + pNdefRecord[2] +
                pNdefRecord[3]);
      else
      ...
      /* ID present ?*/
      if (pNdefRecord[0] & NDEF_RECORD_IL_MASK)
        temp = (pNdefRecord + 7 + pNdefRecord[1] + (pNdefRecord[2] << 24) +
                (pNdefRecord[3] << 16) + (pNdefRecord[4] << 8) +
                pNdefRecord[5] + pNdefRecord[6]);
      else
        temp =
            (pNdefRecord + 6 + pNdefRecord[1] + (pNdefRecord[2] << 24) +
             (pNdefRecord[3] << 16) + (pNdefRecord[4] << 8) + pNdefRecord[5]);
    }
  }
  return temp;
}
